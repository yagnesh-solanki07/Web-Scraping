...
const START_URL =
  "https://specsonline.com/product-category/wine/?show=80&orderby=popularity";
  ...
  while (true) {
    ...
    const wines = await page.evaluate(() => {
      return Array.from(document.querySelectorAll(".products li.product")).map(p => ({
        ...
        Id: p.querySelector("a.add_to_cart_button")?.getAttribute("data-product_id")?.trim() || "",
        Name: p.querySelector("h2")?.innerText.trim() || "",
        Price: p.querySelector('strong[aria-label="Price"] ins')?.innerText.trim() || "",
        "Variant-Detail": p.querySelector(".product-size")?.innerText.trim() || ""
